"use client";
/**
 * MintButtonF3 — MAINNET
 * Phase 3: due gruppi di accesso (burn / sol).
 * - burn: tokenBurn 100k $BRIX + solPayment 0.12 SOL
 * - sol:  solFixedFee 0.10 + solPayment 0.12 = 0.22 SOL totale
 * mintLimit per wallet: 5 (guard id=1, CM separata da F1).
 * CM separata da F1 — CANDY_MACHINE_ADDRESS va aggiornato con l'indirizzo F3.
 */
...
const BRIX_BURN_QTY         = 100_000;
const MINT_PRICE            = 0.12;
const BURN_FEE              = 0.10;  // SOL fee extra per gruppo "sol"
const MINTLIMIT_ID          = 1;     // CM separata da F1 — id=1 non conflicta
...
function parseError(msg: string, group?: "burn" | "sol"): string {
  ...
  if (m.includes("mintended") || m.includes("after end date"))     return "Phase F3 has ended.";
...
export default function MintButtonF3() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
      ...
      <div style={{ fontSize: ".6rem", color: "#888870", fontFamily: "var(--font-mono)", letterSpacing: ".1em", textAlign: "center", padding: "4px 0" }}>F3 — CHOOSE ACCESS METHOD</div>
