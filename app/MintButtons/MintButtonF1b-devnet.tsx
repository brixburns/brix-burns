"use client";
/**
 * MintButtonF1bDevnet — DEVNET TEST
 * Phase 1b (Public): due gruppi di accesso (burn / sol).
 * - burn: tokenBurn 25k $BRIX + solPayment 0.05 SOL
 * - sol:  solFixedFee 0.05 + solPayment 0.05 = 0.10 SOL totale
 * mintLimit per wallet: 5 (guard id=2, separato da F1a id=1).
 * Stessa CM di F1a — gli utenti che hanno mintato 2 in F1a possono mintarne altri 5 qui.
 */
...
const CANDY_MACHINE_ADDRESS = "C6P5ZB5tuKgKcHAP2jorhDquHxzKmwFKcwmENQtAWmLs"; // ⚠️ stessa di F1a (la CM è la stessa, cambiano solo i guards)
const COLLECTION_ADDRESS    = "FZYUd99aBvEj6tjDCJYYY7g3BaNYEY5P9uZDep6ERnLC"; // ⚠️ AGGIORNARE
const DESTINATION           = "CQ1vURtsg646tyLC4m1PutK82835CccYVjFkahrE5G6Z";
const BRIX_MINT             = "AYm3eo4dMoZ3DJ1bPGzQy6Pue4eUv8negmYK76sq93oS";
const BRIX_DECIMALS         = 0;
const BRIX_BURN_QTY         = 10;
const MINT_PRICE            = 0.01;
const BURN_FEE              = 0.01;  // SOL fee extra per gruppo "sol"
...
function parseError(msg: string, group?: "burn" | "sol"): string {
  ...
  if (m.includes("mintended") || m.includes("after end date"))     return "Phase F1b has ended.";
...
export default function MintButtonF1bDevnet() {
        ...
        <div style={{ border: `1px solid ${isPartial ? C_ORANGE : C_GREEN}`, padding: "10px 14px", fontSize: ".65rem", color: isPartial ? C_ORANGE : C_GREEN, fontFamily: "var(--font-mono)", lineHeight: 1.7 }}>
          ...
          {assetList.slice(0, 3).map((addr, i) => (
            <div key={addr} style={{ color: "#888870", marginTop: "4px" }}>
              <a href={`https://explorer.solana.com/address/${addr}?cluster=devnet`} target="_blank" rel="noopener noreferrer" style={{ color: C_GOLD, textDecoration: "none" }}>→ #{i + 1} {addr.slice(0, 8)}…{addr.slice(-6)} ›</a>
            </div>
          ))}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
      ...
      <div style={{ fontSize: ".6rem", color: "#888870", fontFamily: "var(--font-mono)", letterSpacing: ".1em", textAlign: "center", padding: "4px 0" }}>F1b — CHOOSE ACCESS METHOD</div>
