...
const en = {
  ...
  faq: [
    ...
    { q: "How does the team earn?",
      a: "Not from the tax: the creator share goes to a contract, half to burn $BRIX and half to the Top Burners prizes. Not from the mint: no free Trixsters. Only a dev buy of 0.5-1% of the supply from the curve at launch, tax paid, with amount and wallet declared." },
    { q: "What are the Top Burners? Is it a lottery?",
      a: "A weekly ranking of who burned the most $BRIX. The top 3 split the prize pool in BNB, 50% / 30% / 20%, at the same address on BNB Chain. No luck involved: the most burned wins, and the ranking is public, computed from chain data. No fixed prize either: the pool is half the creator share, so it depends on trading volume." },
...
const zh: Dict = {
  ...
  faq: [
    ...
    { q: "团队如何获利？",
      a: "不靠交易税：创作者分成进入合约，一半销毁 $BRIX，一半用于 Top Burners 奖金。不靠铸造：团队没有免费的 Trixster。只有上线时开发者从曲线买入 0.5-1% 的供应量，照常缴税，金额和钱包公开声明。" },
    { q: "什么是 Top Burners？是抽奖吗？",
      a: "每周销毁 $BRIX 最多的排行榜。前 3 名以 BNB 瓜分奖金池：50% / 30% / 20%，发送至 BNB Chain 上的同一地址。不靠运气：销毁最多者胜，排名公开，根据链上数据计算。奖金也不固定：奖金池来自创作者分成的一半，取决于交易量。" },
